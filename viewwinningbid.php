...
$sqlproduct = "SELECT * FROM winners LEFT JOIN product ON winners.product_id = product.product_id LEFT JOIN customer ON winners.customer_id=customer.customer_id where (winners.status='Pending' OR winners.status='Active') AND winners.customer_id='$_SESSION[customerid]'  ORDER BY winners.winner_id DESC ";
...
<b>Amount payable: Rs. <?php echo $rsproduct[winning_bid]; ?>
<hr>
<a href='paywinningbid.php?winner_id=<?php echo $rsproduct[winner_id]; ?>'><b>Claim winning bid</b></a>

<?php
}
else if($rsproduct[6] == "Active")
{
?>
