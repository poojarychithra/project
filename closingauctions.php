<?php
include("header.php");
$adddate = date('Y-m-d', strtotime("+1 days"));
?>
<script>
	...
	<div class="banner">
		...
		<div class="w3l_banner_nav_right">
			<div class="w3l_banner_nav_right_banner3">
				<h3><?php echo $_GET[auctiontype];?></h3>
			</div>
		...
		while($rscategory = mysqli_fetch_array($qsqlcategory))
		{
			...
			$sqlproductcount = "select * from product WHERE status='Active' AND category_id='$rscategory[category_id]'  AND end_date_time>'$dt $tim' AND end_date_time <'$dt 23:59:59' "; 
			if($_GET[auctiontype] == "LatestAuctions")
			{
				
				$sqlproductcount = $sqlproductcount  . " order by product_id DESC limit 0,3";
			}
			else
			{
			...
			$qsqlproductcount = mysqli_query($con,$sqlproductcount);
			if(mysqli_num_rows($qsqlproductcount) >=1)
			{
		?>	
				<div class="w3ls_w3l_banner_nav_right_grid1">
					<h6><a href='allproducts.php?category_id=<?php echo $rscategory[0]; ?>'><?php echo $rscategory[category_name]; ?></a></h6>
			<?php
			$sqlproduct = "select * from product WHERE status='Active' AND category_id='$rscategory[category_id]'  AND end_date_time>'$dt $tim' AND end_date_time <'$adddate 23:59:59' "; 
			if($_GET[auctiontype] == "LatestAuctions")
			{
...
<fieldset>
	<a href='single.php?productid=<?php echo $rsproduct[product_id]; ?>'><input type="button" name="submit" value="Bid Now" class="button" /></a>			
			...
			?>
				...
				&nbsp; &nbsp; <a href="allproducts.php?category_id=<?php echo $rscategory[0]; ?>"><b>View All...</b></a>
				<hr>
		<?php
			}
		}
