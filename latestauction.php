...
<!-- banner -->
	<div class="banner">
		...
		?>	
			...
			<?php
			$sqlproduct = "select * from product WHERE status='Active' AND end_date_time>'$dt $tim'  AND start_date_time<'$dt $tim' AND  category_id='$rscategory[category_id]' ";
			if($_GET[auctiontype] == "Latest Auctions")
			{
				
				$sqlproduct = $sqlproduct  . " order by start_date_time DESC limit 0,3 ";
			}
			else
			{
...
<!-- Timer code ends here -->
											...
											<h4>Started on <?php echo date("d-M-Y h:i A",strtotime($rsproduct[start_date_time]));?></h4><br>
